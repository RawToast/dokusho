...
import cats.effect.{IO, Sync, Concurrent}
...
trait Http4sRouter
  ...
  override def jsonDecoder[F[_]: Concurrent]: EntityDecoder[F, Json] =
    CirceInstances.builder.build.jsonDecoder

  implicit def decoders[F[_]: Concurrent, A: Decoder]: EntityDecoder[F, A] =
    jsonOf[F, A]

  implicit class EncoderOpz[A](val wrappedEncodeable: A):
    final def asJson(implicit encoder: Encoder[A]): Json = encoder(
      wrappedEncodeable
    )
